...
QVariant BanTableModel::data(const QModelIndex &index, int role) const
{
    ...
    const CBannedNodeRow &row = bans[index.row()];
    switch (index.column())
    {
    case Address:
        return QString::fromStdString(row.addr);
    case BannedUntil:
        return QDateTime::fromTime_t((uint)row.bannedUntil).toString(Qt::DefaultLocaleShortDate);
    }
    return QVariant();
}
